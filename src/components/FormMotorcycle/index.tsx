...
import { useMotocycleData } from '../../hooks/useMotocyleData'
...
export function FormMotorcycle({ id }: FormMotorcycleProps) {
  ...
  const { data: motorcycleData, isLoading, isError } = useMotocycleData(id)

  useEffect(() => {
    if (motorcycleData) {
      setFormMotorcycleData(motorcycleData)
    }
  }, [motorcycleData])
  // useEffect(() => {
  //   if (id) {
  ...
  // const { data, isPending, isError } = useQuery('data', FormMotorcycle)
  // console.log(data)
  ...
  if (isError) {
    return <p>Erro ao carregar dados da moto.</p>
  }
      ...
      <form onSubmit={handleSubmit}>
        <FormGroup>
          <fieldset>
            ...
            <Input placeholder="#" value={id ? id : ''} disabled></Input>
          </fieldset>
        </FormGroup>
        <FormGroup>
          <fieldset>
